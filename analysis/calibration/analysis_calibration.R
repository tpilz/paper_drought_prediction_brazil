...
# output: Barplot of predictor importances
file_varimp <- "plots/predictor_importance.pdf"

# output: pdf plot of predictor occurrences in regression trees' leaf nodes
# (sumed up over whole forest)
...
# function takes an object of class 'RandomForest' (of package party) and
# returns a df of extracted classified predictor realisations of the nodes
# with highest and lowest median responses
get_forest_dat <- function(forest_obj) {
  map_dfr(forest_obj@where, function(x) {
    data.frame(y = forest_obj@responses@variables$resp_val, node = x) %>%
      group_by(node) %>%
      # median of response for each node and count number of values per node
      summarise(y_med = median(y), count = n()) %>%
      # min and max values of median responses (identify best and worst nodes)
      ...
      ungroup() %>%
      select(node, y_med, group) %>%
      # join predictor realisations to min/max nodes
      left_join(.,
                bind_cols(node = x, forest_obj@data@get("input")),
                by = "node") %>%
      # classify predictor values
      mutate(diff_class = factor(diff_class, c("fall", "rise"), c(1,2), ordered = T),
      ...
      gather(key = predictor, value = pred_val, -y_med, -node, -group)
  }, .id = "tree") %>%
    # count predictor value occurrences over all trees (min/max nodes)
    group_by(group, predictor) %>%
    mutate(pred_count = n()) %>%
    group_by(group, predictor, pred_val) %>%
    summarise(pred_val_count = n(),
           pred_val_count_rel = pred_val_count / unique(pred_count) *100)
}
...
dat_analysis_tree_numeric <- dat_response %>%
  ungroup() %>%
  select(KGE, COR, BIAS, VAR, diff_class, area_up, capacity, res_no_up,
         prec_reg, prec_maxday, prec_prev12, prec_prev36) %>%
  mutate_if(~ !is.numeric(.), as.factor) %>%
  # set all "unacceptable" KGE to zero (easier for regression tree as we are not
  # interested in how bad bad values are exactly)
...
# construct forest of regression trees and extract relevant data using helper function (see above)
dat_forests_raw <- dat_analysis_tree_numeric %>%
  ...
  mutate(tree = map(data, ~ cforest(resp_val ~ diff_class + area_up + capacity + res_no_up +
                                    prec_reg+prec_maxday+prec_prev12+prec_prev36,
                                  data = ., controls = cforest_unbiased(ntree = 1000))),
         # calculate importances
         imp = map(tree, function(x) {
                     imp_t <- varimp(x, conditional = T)
                     data.frame(predictor = names(imp_t), imp = imp_t)
                   }),
         # get predictor realisation statistics of leaf nodes with min/max response realisations
         res = map(tree, get_forest_dat))

# extract data and prepare for analysis/plots
dat_forests <- dat_forests_raw %>%
  unnest(imp) %>%
  ...
  mutate(
    ...
    pred_label = case_when(
      predictor == "area_up" ~ "A['up']",
      predictor == "capacity" ~ "V['cap']",
      predictor == "res_no_up" ~ "n['resup']",
      predictor == "diff_class" ~ "paste(Delta['vol'])",
      predictor == "prec_prev12" ~ "P['12']",
      predictor == "prec_prev36" ~ "P['36']",
      predictor == "prec_reg" ~ "P['reg']",
      predictor == "prec_maxday" ~ "P['max']"
    ),
    # sort predictors and response
    pred_label = factor(pred_label, c("V['cap']", "A['up']", "n['resup']", "paste(Delta['vol'])",
                                    "P['max']", "P['reg']", "P['12']", "P['36']")),
    response = factor(response, c("KGE", "COR", "BIAS", "VAR"))
  )
...
# variable importance
dat_plot <- dat_forests %>%
  distinct(pred_label, response, imp)

gp <- ggplot(dat_plot, aes(y = imp, x = pred_label)) +
  geom_col(position = "dodge", width = 2/3) +
  labs(x = "Predictor", y = "Variable importance") +
  scale_x_discrete(labels=parse(text=levels(dat_plot$pred_label))) +
  facet_grid(response ~ ., scales = "free_y") +
  theme_bw(base_size = 20)
ggsave(file_varimp, gp, width=10, height=12)

# predictor realisations for highest/lowest leaf nodes
dat_plot_t <- dat_forests %>%
  # relevant columns and rows (filter relevant response variables)
  select(response, group, predictor, pred_label, pred_val, pred_val_count_rel) %>%
  ...
  complete(pred_val=1:max(no_vals_max), nesting(response, group, predictor, pred_label), fill = list(pred_val_count_rel = 0)) %>%
  ungroup() %>%
  select(-no_vals_max) %>%
  # manual selection of most important predictors
  filter(predictor %in% c("capacity", "area_up", "prec_prev12", "prec_prev36"))

gp <- ggplot(dat_plot_t, aes(x = factor(pred_val, 1:4, labels = c("min", "low", "high", "max")),
                             y = group, fill = pred_val_count_rel)) +
  geom_tile(colour="white", size=1) +
  scale_fill_gradient(low='white', high='grey20') +
  facet_grid(factor(response, c("KGE", "COR", "BIAS", "VAR")) ~ factor(pred_label, c("V['cap']", "A['up']", "P['12']", "P['36']")),
